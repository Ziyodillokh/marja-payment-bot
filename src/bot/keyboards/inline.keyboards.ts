...
export const payInlineKeyboard = (): InlineKeyboard =>
  new InlineKeyboard().text("💳 To'lov qilish", 'pay');
...
export const reviewInlineKeyboard = (paymentId: number): InlineKeyboard =>
  new InlineKeyboard()
    .text('✅ Tasdiqlash', `approve:${paymentId}`)
    .text('❌ Rad etish', `reject:${paymentId}`);

// Approve confirmation
export const approveConfirmKeyboard = (paymentId: number): InlineKeyboard =>
  new InlineKeyboard()
    .text('✅ Ha, tasdiqlash', `approve_confirm:${paymentId}`)
    .text('↩️ Bekor qilish', `approve_cancel:${paymentId}`);

// Reject confirmation
export const rejectConfirmKeyboard = (paymentId: number): InlineKeyboard =>
  new InlineKeyboard()
